...
"""本程序会按照固定的次数进行加强和减少"""
"""如果想改变固定增加或减少的次数，请修改WEIGHT_INCREASE和WEIGHT_DECREASE"""
...
def update_operators_increase_weight(problems, operator_data, condition_type, weight_increase=1):
    """
    增加权重：根据条件为匹配正确的算子增加关键词权重（固定权重增加）
    condition_type: 'file2' 或 'file3'
    weight_increase: 每个匹配到的关键词增加的权重值
    """
    # 创建一个映射：算子名称 -> 索引位置
    operator_index_map = {}
                ...
                if matched_keywords:
                    ...
                    for keyword, info in matched_keywords.items():
                        ...
                        if keyword_idx >= 0:
                            ...
                            print(f"        关键词 '{keyword}': 权重从 {old_weight} 增加到 {new_weight}")
                        else:
                            # 关键词不存在，添加新关键词
                            new_keyword_dict = {
                                "key_word": keyword,
                            ...
                            print(f"        关键词 '{keyword}': 添加新关键词，权重为 {weight_increase}")
...
def update_operators_decrease_weight(problems, operator_data, condition_type, weight_decrease=1):
    """
    减少权重：对于错误匹配的算子，减少其关键词的权重
    condition_type: 'file2' 或 'file3'
    weight_decrease: 每个匹配到的关键词减少的权重值
    """
    # 创建一个映射：算子名称 -> 索引位置
    operator_index_map = {}
    for idx, item in enumerate(operator_data):
                ...
                if matched_keywords:
                    ...
                    for keyword, info in matched_keywords.items():
                        ...
                        if keyword_idx >= 0:
                            ...
                            elif old_weight == 1 and match_count > 5:
...
def process_fixed_weight_all_files():
    """
    处理所有文件：固定权重增加/减少版本
    """
    # 文件路径设置
    ...
    WEIGHT_INCREASE = 2  # 每个匹配到的关键词增加的权重值
    WEIGHT_DECREASE = 1  # 每个匹配到的关键词减少的权重值
    
    print("="*80)
    print("固定权重增加/减少版本 - 处理所有文件")
    print("="*80)
    ...
    # 1. 减少权重：对于错误匹配的算子
    print("\n1. 减少权重（错误匹配的算子）:")
    file2_data_after_decrease, stats_file2_decrease = update_operators_decrease_weight(
        problems, file2_data, 'file2', WEIGHT_DECREASE
    )
    
    # 2. 增加权重：对于正确匹配的算子
    print("\n2. 增加权重（正确匹配的算子）:")
    updated_file2_data, stats_file2_increase = update_operators_increase_weight(
        problems, file2_data_after_decrease, 'file2', WEIGHT_INCREASE
    )
    ...
    # 1. 减少权重：对于错误匹配的算子
    print("\n1. 减少权重（错误匹配的算子）:")
    file3_data_after_decrease, stats_file3_decrease = update_operators_decrease_weight(
        problems, file3_data, 'file3', WEIGHT_DECREASE
    )
    
    # 2. 增加权重：对于正确匹配的算子
    print("\n2. 增加权重（正确匹配的算子）:")
    updated_file3_data, stats_file3_increase = update_operators_increase_weight(
        problems, file3_data_after_decrease, 'file3', WEIGHT_INCREASE
    )
    ...
    print(f"  增加权重 - 每个关键词增加的权重: {WEIGHT_INCREASE}")
    ...
    # 保存完整结果
    full_output_path = "operator2/full_result_fixed_weight.json"
...
if __name__ == "__main__":
    process_fixed_weight_all_files()